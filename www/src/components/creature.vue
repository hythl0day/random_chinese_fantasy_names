      ...
      <div class="col-4">
        <div class="btn-group mb-3 dropup">
          <button
            ...
            data-mdb-toggle="dropdown"
            aria-expanded="false"
          >类别：{{ category == null ? '随机' : creatureCategoryNames[category] }}</button>
          <ul class="dropdown-menu dropdown-menu-end force-scroll">
            <li v-for="item of categoryOptions">
              <a
                class="dropdown-item"
                @click="category = item == '随机' ? null : item"
              >{{ item != '随机' ? creatureCategoryNames[item] : item }}</a>
            </li>
          </ul>
        </div>
      </div>
      <div class="col-4 text-start">
        <div class="btn-group mb-3 dropup">
          <button
            ...
            data-mdb-toggle="dropdown"
            aria-expanded="false"
          >等级：{{ rarity == null ? '随机' : rarityNames[rarity] }}</button>
          <ul class="dropdown-menu dropdown-menu-end force-scroll">
            <li v-for="item of rarityOptions">
              <a
                class="dropdown-item"
                @click="rarity = item == '随机' ? null : item"
                :style="{ color: rarityColors[item] }"
              >{{ item != '随机' ? rarityNames[item] : item }}</a>
            </li>
          </ul>
        </div>
      </div>
